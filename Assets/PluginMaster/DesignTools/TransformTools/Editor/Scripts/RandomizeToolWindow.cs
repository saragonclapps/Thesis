        ...
        protected virtual void OnGUIValue()
        {
            using (new GUILayout.VerticalScope(EditorStyles.helpBox)) //X
            {
                using (var toggleGroup = new EditorGUILayout.ToggleGroupScope("Randomize X", _data.x.randomizeAxis))
                {
                    ...
                    {
                        GUILayout.FlexibleSpace();
                        _data.x.offset.min = EditorGUILayout.FloatField("min:", _data.x.offset.min,
                            EditorStyles.textField);
                        GUILayout.Space(8);
                        _data.x.offset.max = EditorGUILayout.FloatField("max:", _data.x.offset.max,
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox)) //Y
            {
                using (var toggleGroup = new EditorGUILayout.ToggleGroupScope("Randomize Y", _data.y.randomizeAxis))
                {
                    ...
                    {
                        GUILayout.FlexibleSpace();
                        _data.y.offset.min = EditorGUILayout.FloatField("min:", _data.y.offset.min,
                            EditorStyles.textField);
                        GUILayout.Space(8);
                        _data.y.offset.max = EditorGUILayout.FloatField("max:", _data.y.offset.max,
                            EditorStyles.numberField);
                    }
                }
            }
            ...
            using (new GUILayout.VerticalScope(EditorStyles.helpBox)) //Z
            {
                using (var toggleGroup = new EditorGUILayout.ToggleGroupScope("Randomize Z", _data.z.randomizeAxis))
                {
                    ...
                    {
                        GUILayout.FlexibleSpace();
                        _data.z.offset.min = EditorGUILayout.FloatField("min:", _data.z.offset.min,
                            EditorStyles.textField);
                        GUILayout.Space(8);
                        _data.z.offset.max = EditorGUILayout.FloatField("max:", _data.z.offset.max,
                            EditorStyles.numberField);
                    }
                }
            }
        ...
        protected override void OnGUIValue()
        {
            ...
            else
            {
                ...
                using (new GUILayout.VerticalScope(EditorStyles.helpBox))
                {
                    ...
                    {
                        GUILayout.FlexibleSpace();
                        _data.x.offset.min = _data.y.offset.min = _data.z.offset.min
                            = EditorGUILayout.FloatField("min:", _data.x.offset.min, EditorStyles.textField);
                        GUILayout.Space(8);
                        _data.x.offset.max = _data.y.offset.max = _data.z.offset.max
                            = EditorGUILayout.FloatField("max:", _data.x.offset.max, EditorStyles.numberField);
